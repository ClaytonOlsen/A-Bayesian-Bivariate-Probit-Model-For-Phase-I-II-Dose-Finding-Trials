...
JL_simulation = function(starter_dose, pe_true, pt_true, pe_prior, pt_prior, posterior_function_E, posterior_function_T, cohorts, cohort_size, burnin ,nsteps, nsims, Ce, Ct){
  ...
  while (count<nsims) {
    ...
    while (cohort > 0) {
      ...
      #check for dose levels that do not fit inequalities presented under stopping rule in paper
      
      for (item in sort(unique(max_col))){
        if (pe_stop[item] > Ce|pt_stop[item] > Ct){
          max_col = max_col[max_col != item]
        }else{
          max_col = max_col
        }
      }
      #Max_util is mode of max_col
      if (length(max_col) == 0){
        max_util_dose = 0
        break
      }
      ...
      #Selects next dose from sampling group with probability equal to the probability each dose is the dose of maximum utility
    ...
    #After all data has been collected, the dose of highest utility is picked amongst test dose levels
    best_dose_selection = unique(tested_levels)
    last_max_col = c()
    
    for (i in best_dose_selection){
      last_max_col = list.append(last_max_col, max_col[max_col == i])
    }
    final_dose = Mode(last_max_col)
    if (is.na(final_dose)){
      final_dose = 0
    }
