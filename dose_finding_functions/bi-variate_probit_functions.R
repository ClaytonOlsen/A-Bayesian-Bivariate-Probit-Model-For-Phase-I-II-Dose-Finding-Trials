...
p_posterior = function(alpha_E, alpha_T, y_tild_E, y_tild_T, p, ye, yt){
  ...
  prod(post_prod) * dbeta(p, ap, bp)
}

randomvalue_generator_probit <- function(x) {rtruncnorm(1, a=0, b=1, x, 0.2)}
...
run_MCMC_rho <- function(alpha_E, alpha_T, y_tild_E, y_tild_T, p, ye, yt, f, randomvalue, nsteps) {
  ...
  for (i in 1:nsteps){
    total = total + 1
    p_OLD = p
    p <-  step1_rho(p_posterior, randomvalue, alpha_E, alpha_T, y_tild_E, y_tild_T, p, ye, yt)
    p_samples[i] = p
    p_NEW = p
...
gibbs_process = function(alpha_E, alpha_T, y_tild_E, y_tild_T, p, ye, yt, nsteps){
  ...
  p <-  run_MCMC_rho(alpha_E, alpha_T, y_tild_E, y_tild_T, p, ye, yt, f, randomvalue = randomvalue_generator_probit, nsteps = new_nsteps)
