...
posterior_function_E = function(theta_E, ye, level) {
  dose_post = c()
  one_minus_formE = c( (1 - exp(theta_E[1])/(1+exp(theta_E[1]))), (1 - exp(theta_E[2])/(1+exp(theta_E[2]))), (1 - exp(theta_E[3])/(1+exp(theta_E[3]))), (1 - exp(theta_E[4])/(1+exp(theta_E[4]))), (1 - exp(theta_E[5])/(1+exp(theta_E[5])))   )
  
  for (j in level:5){
    for (i in 1:length(ye[[j]])){
      post_value = (1 - prod(one_minus_formE[1:j]))^ye[[j]][i] * prod(one_minus_formE[1:j])^(1 - ye[[j]][i])
...
posterior_function_T = function(theta_T, yt, level) {
  dose_post = c()
  one_minus_formT = c( (1 - exp(theta_T[1])/(1+exp(theta_T[1]))), (1 - exp(theta_T[2])/(1+exp(theta_T[2]))), (1 - exp(theta_T[3])/(1+exp(theta_T[3]))), (1 - exp(theta_T[4])/(1+exp(theta_T[4]))), (1 - exp(theta_T[5])/(1+exp(theta_T[5])))   )
  
  for (j in level:5){
    for (i in 1:length(yt[[j]])){
      post_value = (1 - prod(one_minus_formT[1:j]))^yt[[j]][i] * prod(one_minus_formT[1:j])^(1-yt[[j]][i])
      dose_post = list.append(dose_post, post_value)
    }
  }
...
JL_simulation = function(starter_dose, pe_true, pt_true, pe_prior, pt_prior, posterior_function_E, posterior_function_T, cohorts, cohort_size, burnin ,nsteps, nsims, Ce, Ct){
  ...
  while (count<nsims) {
    ...
    while (cohort > 0) {
      ...
      #cat("\nmean pe: ", colMeans(pe_sims))
      #cat("\nmean pt: ",colMeans(pt_sims))
      ...
      #sample_mean_utils = 2*(1.42+mean_util)
      ...
      #cat("\nmean util: ", mean_util)
      #cat("\nadjusted:", sample_mean_utils)
      max_util_dose = which.max(mean_util)
      #cat('\nmax util_level: ', max_util_dose)
      ...
      #cat("\ndose sample", dose_sample)
      #cat("\nprobs", dose_sample_counts/sum(dose_sample_counts), "\n\n")
    ...
    #cat("best_selection, ", best_dose_selection)
    last_max_col = c()
